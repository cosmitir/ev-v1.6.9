...
public class NodeParent {
    ...
    public final void m1511recursivelyRemovePointerId0FcD4WY(long j) {
        int i = 0;
        while (i < this.children.getSize()) {
            Node node = this.children.getContent()[i];
            node.getPointerIds().remove(PointerId.m1513boximpl(j));
            if (node.getPointerIds().isEmpty()) {
                this.children.removeAt(i);
            } else {
                node.m1511recursivelyRemovePointerId0FcD4WY(j);
                i++;
            }
        }
    }
