...
public final class LayoutTreeConsistencyChecker {
    ...
    private final boolean consistentLayoutState(LayoutNode layoutNode) {
        ...
        if (!layoutNode.isPlaced()) {
            ...
            if (!Intrinsics.areEqual((Object) (parent$ui_release == null ? null : Boolean.valueOf(parent$ui_release.isPlaced())), (Object) true)) {
                return true;
            }
        }
        if (layoutNode.getLayoutState$ui_release() == LayoutNode.LayoutState.NeedsRemeasure && this.postponedMeasureRequests.contains(layoutNode)) {
            return true;
        }
        LayoutNode.LayoutState layoutState$ui_release = parent$ui_release != null ? parent$ui_release.getLayoutState$ui_release() : null;
        return layoutNode.getLayoutState$ui_release() == LayoutNode.LayoutState.NeedsRemeasure ? this.relayoutNodes.contains(layoutNode) || layoutState$ui_release == LayoutNode.LayoutState.NeedsRemeasure || layoutState$ui_release == LayoutNode.LayoutState.Measuring : layoutNode.getLayoutState$ui_release() != LayoutNode.LayoutState.NeedsRelayout || this.relayoutNodes.contains(layoutNode) || layoutState$ui_release == LayoutNode.LayoutState.NeedsRemeasure || layoutState$ui_release == LayoutNode.LayoutState.NeedsRelayout || layoutState$ui_release == LayoutNode.LayoutState.Measuring || layoutState$ui_release == LayoutNode.LayoutState.LayingOut;
    }
    ...
    private static final void logTree$printSubTree(LayoutTreeConsistencyChecker layoutTreeConsistencyChecker, StringBuilder sb, LayoutNode layoutNode, int i) {
        ...
        if (nodeToString.length() > 0) {
            if (i > 0) {
                ...
                StringBuilder append2 = sb.append(nodeToString);
