...
public final class TrieNode<E> {
    ...
    private final TrieNode<E> mutableCollisionAddAll(TrieNode<E> trieNode, DeltaCounter deltaCounter, MutabilityOwnership mutabilityOwnership) {
        ...
        while (i < objArr2.length) {
            CommonFunctionsKt.m33assert(i2 <= i);
            if (!collisionContainsElement((E) objArr2[i])) {
                copyOf[length + i2] = objArr2[i];
                i2++;
                CommonFunctionsKt.m33assert(length + i2 <= copyOf.length);
            }
            i++;
        }
    ...
    private final Object mutableCollisionRetainAll(TrieNode<E> trieNode, DeltaCounter deltaCounter, MutabilityOwnership mutabilityOwnership) {
        ...
        while (true) {
            ...
            if (trieNode.collisionContainsElement((E) objArr2[i])) {
                ...
                i2++;
                CommonFunctionsKt.m33assert(0 + i2 <= objArr.length);
            }
            i++;
        }
        ...
        if (this == otherNode) {
            ...
            while (i5 != 0) {
                ...
                if (hasNoCellAt(lowestOneBit)) {
                    makeNode = (TrieNode<E>) otherNode.getBuffer()[indexOfCellAt$runtime_release2];
                } else if (otherNode.hasNoCellAt(lowestOneBit)) {
                    makeNode = getBuffer()[indexOfCellAt$runtime_release];
                } else {
                    E e = (E) getBuffer()[indexOfCellAt$runtime_release];
                    ...
                    if (!z || !z2) {
                        if (z) {
                            ...
                            i2 = lowestOneBit;
                            makeNode = makeNode(e == null ? 0 : e.hashCode(), e, e2 == null ? 0 : e2.hashCode(), e2, i + 5, mutator.getOwnership$runtime_release());
                            buffer[i6] = makeNode;
                            i6++;
                            i5 ^= i2;
                        }
        ...
        if (this == otherNode) {
            ...
            if (bitCount == 1 && i != 0) {
                ...
                while (i6 < objArr2.length) {
                    CommonFunctionsKt.m33assert(i7 <= i6);
                    if (objArr2[i6] != Companion.getEMPTY$runtime_release()) {
                        objArr[0 + i7] = objArr2[i6];
                        i7++;
                        CommonFunctionsKt.m33assert(0 + i7 <= bitCount);
                    }
                    i6++;
                }
        ...
        if (this == otherNode) {
            ...
            if (i4 == this.bitmap) {
                ...
                while (i6 < objArr3.length) {
                    CommonFunctionsKt.m33assert(i5 <= i6);
                    if (objArr3[i6] != Companion.getEMPTY$runtime_release()) {
                        objArr2[0 + i5] = objArr3[i6];
                        i5++;
                        CommonFunctionsKt.m33assert(0 + i5 <= bitCount);
                    }
                    i6++;
                }
                return new TrieNode(i4, objArr2, mutator.getOwnership$runtime_release());
            }
        }
    }
