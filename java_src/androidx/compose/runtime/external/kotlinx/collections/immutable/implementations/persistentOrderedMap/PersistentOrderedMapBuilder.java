...
public final class PersistentOrderedMapBuilder<K, V> extends AbstractMutableMap<K, V> implements PersistentMap.Builder<K, V> {
    ...
    public PersistentMap<K, V> build() {
        ...
        if (build == this.map.getHashMap$runtime_release()) {
            CommonFunctionsKt.m33assert(this.firstKey == this.map.getFirstKey$runtime_release());
            CommonFunctionsKt.m33assert(this.lastKey == this.map.getLastKey$runtime_release());
    ...
    public V put(K k, V v) {
        ...
        if (linkedValue != null) {
            ...
            CommonFunctionsKt.m33assert(!linkedValue3.getHasNext());
